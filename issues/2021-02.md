...
글의 저자는 온라인에서 달았던 관련 주제에 대한 답글들을 정리해 어떤 경우에 이들이 사용되어야 하고, 무엇이 다른지 그리고 어떤 경우에 사용되어야 하는지를 자세하게 소개한다.
...
- 최소한 한 가지 이상의 까다로운 문제(tricky question) 이 존재한다.
- 일반적으로 한 개 이상의 알고리즘 문제가 있다.
...
튜토리얼을 통해 무제한 대댓글, 이모티콘, 페이지 새로 고침 없이 표현되는 실시간 업데이트, 마크다운 지원 등의 기능이 지원되는 댓글 개발을 따라 해 볼 수 있다.
