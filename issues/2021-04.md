...
최근 가장 인기 있는 음성 기반의 소셜 네트워크 앱인 클럽 하우스는 많은 이들에게 회자되고 있다. 

UX는 FE 관점에서도 매우 주요한 영역 중 하나이기 때문에, 클럽 하우스의 UX 사례 분석을 통해 좋은 UI/UX를 적용할 수 있는 인사이트를 얻을 수 있지 않을까?
...
리액트에서 유지보수하기 쉬운 컴포넌트 테스트 코드를 작성하는 것은 매우 어려운 일이다. 이 글에서는 다음 세 단계로 이루어진 패턴을 통해 가독성이 좋은 컴포넌트 테스트를 작성하는 방법에 대해 설명한다.
...
- 3. `bind` 메서드를 사용하는 경우
- 4. `call`, `apply` 메서드를 사용하는 경우
- 5. 상위 객체에서 함수를 호출하는 경우
- 6. 함수 혹은 상위 스코프가 `strict mode`인 경우
- 7. 그 외 경우

## [CSS-in-JS support in DevTools](https://developers.google.com/web/updates/2021/02/css-in-js)

<img src="https://developers.google.com/web/updates/images/2021/02/css-in-js/mechanism.svg" width=500>

구글은 Chrome 85 버전부터 [개발자도구](https://developers.google.com/web/tools/chrome-devtools)에서 CSS-in-JS 프레임워크를 위한 스타일 수정 기능을 지원했다. 개발자도구의 스타일 창에서 어떻게 CSS-in-JS 규칙 수정 기능을 추가했는지 설명하는 글이다.
...
<img src="https://www.freecodecamp.org/news/content/images/size/w2000/2021/02/how-to-fetch-data-in-react.png" width=500>

이 글은 리액트에서 HTTP 요청을 통해 데이터를 fetch하는 방법 다섯 가지를 예제 코드와 함께 설명한다.
...
브라우저에서 리액트 컴포넌트를 클릭하면 로컬 IDE의 해당 컴포넌트 코드로 편리하게 이동시켜주는 도구다.
