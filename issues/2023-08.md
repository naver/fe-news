...
지난 7년간 React의 아이코닉 한 개발자로 활동했던 Dan Abromov가 Meta에서 퇴사할 계획이라고 밝혔다.
...
이 글에서 제시한 해결책은 한 번에 여러 계층의 추상화를 제공해야 한다는 것이다. 단순히 데이터를 패싱하고 렌더링 하는 config 방식, 특정 컴포넌트를 전달하는 slot 방식, 하위 컴포넌트 및 스타일링을 직접 위임하는 cutom 방식 이 세 가지의 추상 계층으로 컴포넌트의 활용도와 재사용성의 향상을 가져올 수 있다고 한다.
...
JavaScript 코드를 [WebAssembly Component](https://github.com/WebAssembly/component-model)를 생성해 준다.
