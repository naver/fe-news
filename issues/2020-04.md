...
React Router의 차기 메이저 버전인 v6의 새로운 변화들을 소개한다. 가장 큰 변화로는 번들 크기가 이전 버전(v5)에 비해 ~70% 용량이 감소되었고, 새로운 Hooks와 컴포넌트들의 변화 들의 내용을 설명한다.

> 현재 v6의 최신 버전은 지난 2월 공개된 [v6.0.0-alpha.2](https://github.com/ReactTraining/react-router/releases/tag/v6.0.0-alpha.2) 이다.
...
맞춤법 검사를 원하는 단어나 문장을 입력해 주세요. Redux Toolkit(이전 프로젝트명 "Redux Starter Kit")은 모범사례가 적용된 표준적인 Redux 로직 작성 제공을 목표로 하며, Redux 사용 시 겪게 되는 다음의 보편적인 문제들을 해결하기 위해 개발되었다.
- Redux 스토어 설정의 복잡성
